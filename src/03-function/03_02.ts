import { CityType, GovBuildingsType, HouseType } from '../02-objects/02_02';


export const addMoneyToBudget = (arrBuildings: GovBuildingsType[], type: string, count: number) => {
  return arrBuildings.filter(b => b.type === type).map(el => el.budget += count)
}
